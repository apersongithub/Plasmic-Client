	...
	virtual void onTick(Gamemode* gm) override {
		...
		if (game::minecraftGame->networkHandler->raknetInstance->isonaServer() == true)
			if (game::localPlayer->gamemode != 1) {
				Disable();
				ClickGui::sendNotificationf(mltext("You can only use %s in a world or when you are in creative.", "Vous pouvez utiliser %s seulement dans les mondes ou quand vous etes en creatif."), name);
			}
	}
